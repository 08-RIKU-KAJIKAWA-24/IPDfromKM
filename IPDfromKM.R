...
path <- "C:\\Users\\Riku Kajikawa\\Desktop\\Never Give Up\\野村ゼミ\\野村Aゼミ\\データ抽出\\"
...
digizeit <- read.table(paste(path, digisurvfile, sep=""), header=TRUE, sep=",")
t.S <- digizeit[,2]
S <- digizeit[,3]

# Read in published numbers at risk, n.risk, at time, t.risk, lower and upper
# indexes for time interval
pub.risk <- read.table(paste(path, nriskfile, sep=""), header=FALSE, sep=",")
t.risk <- pub.risk[,2]
lower <- pub.risk[,3]
upper <- pub.risk[,4]
n.risk <- pub.risk[,5]
...
write.table(matrix(c(t.S[1:n.t], n.hat[1:n.t], d, cen), ncol=4, byrow=F), paste(path, KMdatafile, sep=""), sep=",")
...
data0 <- read.csv("C:\\Users\\Riku Kajikawa\\Desktop\\Never Give Up\\野村ゼミ\\野村Aゼミ\\データ抽出\\.csv", header=TRUE, sep=",")
data1 <- read.csv("C:\\Users\\Riku Kajikawa\\Desktop\\Never Give Up\\野村ゼミ\\野村Aゼミ\\データ抽出\\.csv", header=TRUE, sep=",")

# -------------------------------------------------------------------------------------------------------------------
analdata <- merge(data0, data1, all=T)
cox <- coxph(formula = Surv(time, event)~arm, data=analdata)
summary(cox)

analdata <- merge(data0, data1, all=T)
KM.est <- survfit(Surv(time, event)~arm, data=analdata)
KM.est
cox <- coxph(formula = Surv(time, event) ~ arm, data=analdata)
summary(cox)

# KM曲線の描画 -----------------------------------------------------------------------------------------------------
analdata <- read.csv("C:\\Users\\Riku Kajikawa\\Desktop\\Never Give Up\\野村ゼミ\\.csv")
survfit(Surv(time, event)~arm, data=analdata) %>% 
    ggsurvplot(xlim = #x軸の範囲をベクトル指定,
               palette = "Lancet",
               risk.table = "abs_pct",
               conf.int = FALSE,
               ggtheme = theme_gray())
